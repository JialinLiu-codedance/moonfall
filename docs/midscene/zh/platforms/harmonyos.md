...
查看[完整报告](https://lf3-static.bytednsdoc.com/obj/eden-cn/nupipfups/Midscene/1.0-showcases/harmony.html)，或浏览更多 [Midscene 案例](/zh/showcases.md)。
...
Playground 是验证连接的最快方式。无需编写代码，即可体验 `aiAct`、`aiQuery` 和 `aiAssert` 等核心能力。它与 `@midscene/harmony` 共享相同的核心，因此在 Playground 中通过的流程，在脚本中运行会保持一致。
...
本节介绍如何自定义设备行为、把 Midscene 接入独立框架，以及排查 HDC 问题。更多构造函数参数位于 [API 参考的 HarmonyOS 章节](/zh/reference.md#harmonyos)。
...
* HarmonyOS 专属参数与接口：[API 参考（HarmonyOS）](/zh/reference.md#harmonyos)
