...
本篇文档会重点介绍 Midscene 的模型配置参数。如果你对 Midscene 的模型策略感兴趣，请阅读 [模型策略](/zh/model-strategy.md)。如果你想查看常用模型的配置示例，请阅读 [配置你的模型](/zh/model-common-config.md)。

## 必选配置

你需要为 Midscene 配上一个默认模型，详见 [模型策略](/zh/model-strategy.md) 文档。
...
| `MIDSCENE_MODEL_REASONING_ENABLED` | 控制是否启用模型的原生思考能力，Midscene 默认关闭。详见[模型原生的思考模式](/zh/model-strategy.md#模型原生的思考模式) |
| `MIDSCENE_MODEL_REASONING_EFFORT` | 控制模型的原生思考力度，部分模型支持。常用值：`low`、`medium`、`high`。详见[模型原生的思考模式](/zh/model-strategy.md#模型原生的思考模式) |
| `MIDSCENE_MODEL_REASONING_BUDGET` | 思考 token 预算（数字），部分模型支持。详见[模型原生的思考模式](/zh/model-strategy.md#模型原生的思考模式) |
...
你也可以使用 JavaScript 来为每个 Agent 配置模型参数，详见 [API 参考](/zh/reference.md#common)
...
* 如果使用 [`createOpenAIClient`](/zh/reference.md#自定义-openai-客户端) 参数，环境变量方式会被覆盖

如果需要更细粒度的控制（例如只对特定任务启用 LangSmith），可以通过 [`createOpenAIClient`](/zh/reference.md#自定义-openai-客户端) 手动包装客户端。

### 使用 Langfuse

[Langfuse](https://langfuse.com) 是另一个流行的 LLM 可观测性平台。Midscene 已经集成了 Langfuse 的 `observeOpenAI` wrapper，自动追踪所有 OpenAI API 调用。
...
**了解更多**：查看 [Langfuse OpenAI 集成文档](https://langfuse.com/integrations/model-providers/openai-js) 获取更多配置选项和最佳实践。
