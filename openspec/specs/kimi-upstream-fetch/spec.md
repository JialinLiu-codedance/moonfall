...
定义 Moonfall 按上游 lock 拉取并缓存 Kimi Code 源码的行为：remote 身份校验、detached checkout 到 pin commit、clean worktree 要求，以及失败语义与离线模式，使 B0.3 工具链校验与 B1 SEA 构建拥有唯一合法 checkout 输入。
